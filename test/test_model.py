...
def input_data_for_multi_head_attention_layer(request):
    ...
    query_sequences = tf.random.uniform(
        (batch_size, seq_len, d_model), dtype=tf.float32
    )
    key_value_sequences = tf.random.uniform(
        (batch_size, seq_len, d_model), dtype=tf.float32
    )
...
def input_data_for_encoder_layer(request):
    ...
    encoder_sequences = tf.random.uniform(
        (batch_size, seq_len), minval=0, maxval=10000, dtype=tf.int32
    )
...
def input_data_for_decoder_layer(request, decoder_layer_config):
    ...
    # Token sequences
    decoder_sequences = tf.random.uniform(
        (batch_size, seq_len), minval=0, maxval=10000, dtype=tf.int32
    )
    ...
    embedding_layer = tf.keras.layers.Embedding(
        decoder_layer_config["vocab_size"], decoder_layer_config["d_model"]
    )
...
def test_multi_head_attention_layer_initialization(mha_layer):
    ...
    ), "Layer is not a MultiHeadAttention instance."
...
def test_multi_head_attention_layer_forward_pass(mha_layer, mha_input_data):
    ...
    assert (
        mha_layer.d_model == query_sequences.shape[2]
    ), "Will not be able to add residual skip connection otherwise"
    output = mha_layer(query_sequences, key_value_sequences, key_value_sequence_lengths)

    # Assert the output shape is as expected
    expected_shape = (
        query_sequences.shape[0],
        query_sequences.shape[1],
        mha_layer.d_model,
    )
    assert (
    ...
    assert not tf.reduce_all(
        tf.equal(query_sequences, output)
...
def test_feed_forward_layer_forward_pass(ffn_layer, ffn_input_data):
    ...
    # Assert skip connection compatibility
    assert (
        ffn_layer.d_model == sequences.shape[2]
    ), "Will not be able to add residual skip connection otherwise"
...
def test_encoder_layer_forward_pass(encoder_layer, encoder_input_data):
    ...
    # Assert the output shape is as expected
    expected_shape = (
        encoder_sequences.shape[0],
        encoder_sequences.shape[1],
...
def test_decoder_layer_forward_pass(decoder_layer, decoder_input_data):
    ...
    # Assert the output shape is as expected
    expected_shape = (
        decoder_sequences.shape[0],
        decoder_sequences.shape[1],
        decoder_layer.vocab_size,
    )
    assert (
        output.shape == expected_shape
    ), f"Expected output shape {expected_shape}, but got {output.shape}"
